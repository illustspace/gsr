...
contract GeoSpatialRegistry is NativeMetaTransaction, ContextMixin {
  ...
  {
    Placement storage placement = _findValidPlacement(assetId, publisher, true);
  ...
  function isWithin(
    ...
    return
      ...
      (placement.geohash.geohash >>
        (placement.geohash.bitPrecision - boundingGeohash.bitPrecision));
  }
  ...
  function _verifyGeohash(Geohash calldata geohash) internal pure {
    ...
    require(geohash.bitPrecision % 5 == 0, "GSR: Precision not multiple of 5");
  }
